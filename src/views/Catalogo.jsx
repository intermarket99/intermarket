...
/* ============================================================
   ICONOS CON react-icons (Material Design)
   Fijos para tus categorías + automático para nuevas
   ============================================================ */
const ICONOS_CATEGORIAS_FIJOS = {
  accesorios: MdWatch,
  anime: MdSportsEsports,
  calzado: MdShoppingBag,
  juguetes: MdToys,
  mascota: MdPets,
  "ropa americana": MdCheckroom,
  ropa: MdCheckroom,
};
...
const CATALOGO_ICONOS = [
  {
    Icon: MdCheckroom,
    palabras: [
      "ropa", "moda", "vestiment", "camisa", "pantalon", "blusa",
      "vestido", "falda", "chaqueta", "abrigo", "sudadera", "jeans",
    ],
  },
  {
    Icon: MdShoppingBag,
    palabras: [
      "zapato", "calzado", "tenis", "bota", "sandalia", "taco", "zapatilla",
    ],
  },
  {
    Icon: MdPhoneIphone,
    palabras: [
      "electron", "tecnolog", "celular", "telefono", "comput", "laptop",
      "tablet", "gadget", "audifono", "cargador",
    ],
  },
  {
    Icon: MdHome,
    palabras: [
      "hogar", "casa", "mueble", "cocina", "decoracion", "silla",
      "mesa", "sofa", "almohada",
    ],
  },
  {
    Icon: MdRestaurant,
    palabras: [
      "comida", "alimento", "bebida", "restaur", "snack", "cafe", "dulce", "pan",
    ],
  },
  {
    Icon: MdFitnessCenter,
    palabras: [
      "deporte", "fitness", "gym", "futbol", "basquet", "entrenamiento", "pelota",
    ],
  },
  {
    Icon: MdSpa,
    palabras: [
      "belleza", "cosmet", "maquill", "skincare", "perfume", "crema", "cabello",
    ],
  },
  {
    Icon: MdMenuBook,
    palabras: [
      "libro", "educacion", "estudio", "lectura", "cuaderno", "escolar",
    ],
  },
  {
    Icon: MdToys,
    palabras: ["juguete", "nino", "nina", "bebe", "infantil", "juego"],
  },
  {
    Icon: MdWatch,
    palabras: [
      "accesorio", "joya", "reloj", "bolso", "cartera", "collar", "anillo",
    ],
  },
  {
    Icon: MdFavorite,
    palabras: [
      "salud", "medic", "farmac", "vitamina", "bienestar",
      "mascota", "perro", "gato", "pet", "animal",
    ],
  },
  {
    Icon: MdDirectionsCar,
    palabras: ["auto", "vehiculo", "moto", "carro", "repuesto"],
  },
  {
    Icon: MdBuild,
    palabras: ["herramient", "ferreter", "taller", "construccion", "tornillo"],
  },
  {
    Icon: MdMusicNote,
    palabras: ["musica", "instrumento", "audio", "guitarra", "altavoz"],
  },
  {
    Icon: MdPalette,
    palabras: ["arte", "manualidad", "pintura", "dibujo", "craft"],
  },
  {
    Icon: MdLocalFlorist,
    palabras: ["jardin", "planta", "flor", "maceta", "semilla"],
  },
  {
    Icon: MdWork,
    palabras: ["oficina", "papeler", "escritorio", "trabajo"],
  },
  {
    Icon: MdSportsEsports,
    palabras: ["anime", "manga", "otaku", "videojuego", "gamer", "consola"],
  },
  {
    Icon: MdPhotoCamera,
    palabras: ["foto", "camara", "fotografia", "lente"],
  },
  {
    Icon: MdCardGiftcard,
    palabras: ["regalo", "souvenir", "detalle"],
  },
];
...
/** Devuelve el componente de icono según el nombre de la categoría */
const getIconoCategoria = (nombre = "") => {
  const texto = normalizarTexto(nombre);
  if (!texto) return MdCategory;

  if (ICONOS_CATEGORIAS_FIJOS[texto]) {
    return ICONOS_CATEGORIAS_FIJOS[texto];
  }

  let mejor = { Icon: MdCategory, score: 0 };

  for (const item of CATALOGO_ICONOS) {
    let score = 0;
    for (const palabra of item.palabras) {
      if (texto.includes(palabra)) score += palabra.length + 2;
    }
    if (score > mejor.score) mejor = { Icon: item.Icon, score };
  }
...
function Catalogo() {
  ...
  const [itemsCompradosRecientemente, setItemsCompradosRecientemente] =
    useState([]);
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("abrirCarrito", abrirCarrito);
  }, [user?.id]);
  ...
  const cargarProductos = async (paginaSolicitada = 0, nuevaCarga = false) => {
    try {
      if (nuevaCarga) setCargando(true);
      ...
      if (nuevaCarga) setProductos(resultados);
      else setProductos((anteriores) => [...anteriores, ...resultados]);
  ...
  const cargarOpcionesFiltros = async () => {
    ...
    (data || []).forEach((producto) => {
      normalizarArray(producto.tallas).forEach((t) => conjuntoTallas.add(t));
      normalizarArray(producto.colores).forEach((c) => conjuntoColores.add(c));
    });

    const ordenarNatural = (a, b) =>
      a.localeCompare(b, "es", { numeric: true, sensitivity: "base" });

    setTallasDisponibles([...conjuntoTallas].sort(ordenarNatural));
    setColoresDisponibles([...conjuntoColores].sort(ordenarNatural));
  };
  ...
  const mostrarToastCarrito = (producto) => {
    ...
    const variante = [producto.talla_seleccionada, producto.color_seleccionado]
      .filter(Boolean)
      .join(" / ");

    toast.innerHTML = `
      <div class="catalogo-toast-content">
        <span class="catalogo-toast-icon"><i class="bi bi-cart-check-fill"></i></span>
  ...
  const totalCarrito = carrito.reduce(
    (total, producto) =>
      total + parseFloat(producto.precio_venta || 0) * (producto.cantidad || 1),
    0
  );
  ...
  return (
    <main className="catalogo-pwa">
      <style>{`
        ...
        .catalogo-quick-filter-bar::-webkit-scrollbar { display: none; }
        ...
        .catalogo-quick-filter-clear { opacity: 0.78; }
        ...
        .catalogo-quick-filter-options::-webkit-scrollbar { display: none; }
        ...
        @media (max-width: 767.98px) {
          .catalogo-quick-filters { margin-top: -0.4rem; margin-bottom: 0.85rem; }
          .catalogo-quick-filter-bar { gap: 0.45rem; padding-bottom: 0.42rem; }
          .catalogo-quick-filter-btn { min-height: 36px; padding: 0.5rem 0.78rem; font-size: 0.78rem; }
          .catalogo-quick-filter-options { padding-top: 0.65rem; }
          .catalogo-filter-option { min-height: 34px; padding: 0.44rem 0.68rem; font-size: 0.76rem; }
        }
      `}</style>
      ...
      <Container fluid="lg" className="catalogo-pwa-container">
          ...
          <div className="catalogo-search-wrapper">
            <div className="catalogo-search-glass">
              ...
              <input
                ...
                onFocus={() => {
                  if (busqueda.trim().length > 1) setMostrarSugerencias(true);
                }}
        ...
        <section className="catalogo-categories-section">
          ...
          <div className="catalogo-categories-scroll">
            ...
            {categorias.map((categoria) => {
              const Icono = getIconoCategoria(categoria.nombre_categoria);
              return (
                <button
                  type="button"
        ...
        <section className="catalogo-quick-filters">
          <div className="catalogo-quick-filter-bar">
            <button
              type="button"
              className={`catalogo-quick-filter-btn ${
                tallaSeleccionada ? "active" : ""
              }`}
              onClick={() =>
            ...
            >
              <span>
                {tallaSeleccionada ? `Talla: ${tallaSeleccionada}` : "Talla"}
              </span>
              <i
                className={`bi ${
                  filtroAbierto === "talla" ? "bi-chevron-up" : "bi-chevron-down"
                }`}
              ></i>
            </button>
            ...
            >
              <span>
                {colorSeleccionado ? `Color: ${colorSeleccionado}` : "Color"}
              </span>
              <i
                className={`bi ${
                  filtroAbierto === "color" ? "bi-chevron-up" : "bi-chevron-down"
                }`}
              ></i>
            </button>
            ...
            <button
              type="button"
              className={`catalogo-quick-filter-btn ${
                mostrarSoloOfertas ? "active" : ""
              }`}
              onClick={() => setMostrarSoloOfertas((v) => !v)}
            >
              <span>Ofertas</span>
            ...
            <div className="catalogo-quick-filter-options catalogo-color-options">
              ...
              {coloresDisponibles.map((color) => (
                <button
                  type="button"
                  key={color}
                  className={`catalogo-filter-option catalogo-filter-option-color ${
        ...
        <section className="catalogo-offer-banner">
          <div className="catalogo-offer-content">
            ...
            >
              <i
                className={`bi ${
                  mostrarSoloOfertas ? "bi-grid-fill" : "bi-percent"
                }`}
              ></i>
        ...
        <section className="catalogo-products-section">
          ...
          {cargando ? (
            ...
            <div className="catalogo-empty-glass">
              ...
              <p>
                Prueba otra búsqueda o cambia la categoría, talla o color
                seleccionado.
              </p>
            </div>
          ) : (
